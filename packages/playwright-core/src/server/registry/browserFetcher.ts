...
export async function downloadBrowserWithProgressBar(title: string, browserDirectory: string, executablePath: string | undefined, downloadURLs: string[], downloadFileName: string, downloadSocketTimeout: number, force: boolean, family?: number) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= retryCount; ++attempt) {
      ...
      const { error } = await downloadBrowserWithProgressBarOutOfProcess(title, browserDirectory, url, zipPath, executablePath, downloadSocketTimeout, family);
      if (!error) {
 ...
 */
function downloadBrowserWithProgressBarOutOfProcess(title: string, browserDirectory: string, url: string, zipPath: string, executablePath: string | undefined, socketTimeout: number, family?: number): Promise<{ error: Error | null }> {
